...
- 임의 hex 하드코딩 금지: `#3B82F6`, `#1B1C1C` 등 → CSS 변수 사용
  - GREP: `/#[0-9A-Fa-f]{3,6}/`
- 기능적 색상(텍스트·배경·CTA·에러)에 팔레트 스케일 직접 사용 금지 → 시맨틱 토큰 사용
  - ❌ `bg-primary-600` → ✅ `bg-primary`
  - ❌ `text-green-500` (본문) → ✅ `text-on-surface`
- 팔레트 스케일 남용 금지 — 강조·위계·장식 목적에만 허용, 권장 단계(100/200/300/500/700) 외 사용 지양
- `orange-*` / `green-*`를 CTA·주요 액션에 사용 금지 → Primary 계열 사용
- 다크 모드 미대응 단일 색상 지정 금지

## Typography

- 스케일 외 임의 크기 금지: `text-[17px]`, `text-[21px]`, `text-[50px]` 등
  - GREP: `/text-\[(?!(?:96|80|64|44|40|32|24|22|18|16|15|13)px)\d+px\]/`
- 본문·설명에 `font-bold`(700) 남용 금지
- `text-caption`(13px)을 본문 역할에 사용 금지
...
- 360ms 초과 전환 금지
  - GREP: `/duration-\[(?:36[1-9]|3[7-9]\d|[4-9]\d{2}|\d{4,})ms\]/`
